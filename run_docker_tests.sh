...
$RUN 'cd /tmp/openpilot/ && flake8 --select=F $(find . -iname "*.py" | grep -vi "^\./pyextra.*" | grep -vi "^\./panda" | grep -vi "^\./laika" | grep -vi "^\./tools" | grep -vi "^\./external")'
$RUN 'cd /tmp/openpilot/ && pylint --disable=R,C,W $(find . -iname "*.py" | grep -vi "^\./pyextra.*" | grep -vi "^\./laika" | grep -vi "^\./tools" | grep -vi "^\./external"| grep -vi "^\./panda"); exit $(($? & 3))'
